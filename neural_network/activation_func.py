...
def relu(value: array) -> array:
    return np.maximum(0, value)


def relu_deriv(value: array) -> array:
    return (value > 0).astype(float)


def leaky_relu(value: array) -> array:
    return np.maximum(0.02 * value, value)


def leaky_relu_deriv(value: array) -> array:
...
def sigmoid_deriv(value: array) -> array:
    return value * (1 - value)


def activ_deriv(active_func: callable,
                value: array, deriv_map: dict) -> array | int:
...
def get_activation_funcs_by_name(activs: list[str]) -> list[callable]:
