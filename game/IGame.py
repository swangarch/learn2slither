...
class IGame(ABC):
	...
	def handle_step(self, action_idx:int) -> int:
		"""Handle the event in 1 game movement, return 1 if new session is initialised, return 0 otherwise"""
		pass
	...
	@abstractmethod
	def update_display(self, action_idx:int, session:int, epsilon:float, min_explo_rate:float, memo_len:int) -> None:
		"""Update the game display, pass parameters that game needs to visualize. 
		"""
		pass

	@abstractmethod
	def reset(self) -> bool:
		"""Reset game state for each action, return bool to indicate if game is still running."""
		pass
